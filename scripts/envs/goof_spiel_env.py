...
def _run_episode_last(
    ...
    *,
    ...
    if done:
        _log(f"[GT] Game {game_id} ended during strategy forcing at turn {turn_number}.")
        return index, None
    ...
    try:
        ...
        if action_id_parsed not in hand_cards:
            _log(f"Invalid action: {action_id_parsed} not in hand cards: {hand_cards}")
            _log(f"Messages: {messages}")
            invalid_action = True
    except Exception:
        invalid_action = True
        _log(f"Invalid action: {action_to_send}")
