...
def _log(*args, **kwargs):
    if _VERBOSE:
        # flush=True (unless the caller overrides it): when multiple envs run
        # concurrently in their own OS process (generate_trajectories.py's
        # multi-env path), each process's stdout is independently buffered.
        # Without an immediate flush, several lines accumulate before being
        # written in one chunk, and a same-sized chunk from another process
        # can land in the middle of it (no newline boundary preserved) once
        # the underlying pipe write isn't atomic — interleaving entire log
        # blocks mid-line. Flushing on every call keeps each line its own
        # small write, matching the explicit flush=True already used on the
        # "always show up" prints elsewhere in this module.
